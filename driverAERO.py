...
class inputs:
    ...
    def __init__(self, inputFile, deltaT):
        ...
        if self.deltaT != deltaT:
            raise Exception('The ROM has been trained with a time step of '+str(deltaT)+', but a time step of '+str(self.deltT)+' was found in the inputs')

    def __readInputFile(self):
        with open(inputFile, 'r') as file:
            print('Opened structural inputs file ' + self.filenameStru + '.')
...
def main():
    ...
    inputs = inputs(options.inputs, databases[0].deltT)

    # Run the prediction step
    Forces = np.empty((inputs.U.shape()[0],0),dtype=int)
    for i in range(inputs.U.shape()[1]):
        Forces.append(ROM.predict(inputs.U[:,i]))
